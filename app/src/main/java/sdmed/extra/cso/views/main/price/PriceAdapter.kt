...
class PriceAdapter(val lifecycleOwner: LifecycleOwner, val relayCommand: ICommand): FRecyclerAdapter<ListItemPriceBinding, MedicineModel>() {
    override var layoutId = R.layout.list_item_price
    override var lifecycle = lifecycleOwner.lifecycle
    override fun onBindViewHolder(holder: FRecyclerViewHolder<ListItemPriceBinding>, position: Int) {
        holder.binding?.let {
